...
import demoRouteRaw from "@/lib/demo_route.json";
...
interface EnrichedNode extends NodeData { px: number; py: number; }
...
interface RouteBuilding { 건물명: string; 상태: string; steps: StepData[]; }
...
function BottomPanel({ step, stepIdx, total, onPrev, onNext, onBack }: {
...
function BuildingTabs({ buildings, active, onChange }: {
  buildings: RouteBuilding[]; active: number; onChange: (i: number) => void;
}) {
    ...
    <div style={{
      ...
      {buildings.map((b, i) => (
        <button key={i} onClick={() => onChange(i)} style={{
          ...
          background: active === i ? "#111" : "#f0f0f0",
...
export default function TestDispatchPage() {
  ...
  const [bldIdx,   setBldIdx]   = useState(0);
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const demoData: RouteResponse = (demoRouteRaw as any)["건물별경로안내"] as RouteResponse;
  ...
  async function run() {
    ...
    try {
      ...
      const data: RouteResponse = JSON.parse(text);
      if (!Array.isArray(data) || data.length === 0) throw new Error("응답 배열이 비어있습니다.");
      setRouteData(data);
      setApiError(null);
      setBldIdx(0);
      setStepIdx(0);
      ...
      setApiError("⚠️ 서버 연결 실패 — 더미 데이터로 표시 중");
      setBldIdx(0);
      setStepIdx(0);
      setAnimKey(k => k + 1);
      setScreen("result");
      ...
      <main className="min-h-screen bg-gray-50 p-6 font-mono text-sm">
        <div className="max-w-xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            <a href="/test" className="text-xs text-gray-400 hover:text-gray-600">← API Test</a>
            <span className="text-gray-300">/</span>
            <h1 className="text-lg font-bold text-gray-800">dispatch 시각화 테스트</h1>
          </div>

          <section className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              <span className="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded">POST</span>
            ...
            <textarea
              value={requestJson}
              onChange={e => { setRequestJson(e.target.value); setJsonError(null); }}
              rows={18}
              spellCheck={false}
              className={`w-full rounded-lg border px-3 py-2 text-xs bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-300 resize-y ${jsonError ? "border-red-400" : "border-gray-200"}`}
            />
            {jsonError && <p className="text-red-500 text-xs mt-1">{jsonError}</p>}

            <button
              onClick={run}
              className="mt-4 w-full py-3 rounded-xl bg-gray-900 text-white font-bold text-sm hover:bg-gray-700 transition"
            >
              경로 계산 →
            </button>
          </section>
    ...
    return (
      <div style={{
        ...
        <pre style={{
          ...
          wordBreak: "break-all", whiteSpace: "pre-wrap", marginBottom: 20,
        }}>{apiError}</pre>
  ...
  const bdata   = routeData[bldIdx];
  const bldName = bdata.건물명;
  const steps   = bdata.steps ?? [];
  ...
  const floorColor = FLOOR_COLOR[dFloor] ?? MAP_COLOR;
  ...
  function handleBldChange(i: number) {
    setBldIdx(i);
    setStepIdx(0);
    setAnimKey(k => k + 1);
  }
  ...
  return (
    <div style={{
      ...
      <BuildingTabs buildings={routeData} active={bldIdx} onChange={handleBldChange} />
      ...
      <BottomPanel
        ...
        onPrev={() => go(-1)}
        onNext={step.is_last_step ? goBack : () => go(1)}
        onBack={goBack}
      />
    </div>
  );
